...
import { RescheduleAppointmentUseCase } from '@modules/appointments/application/use-cases/appointment/reschedule-appointment.use-case';
...
import { rescheduleAppointmentPayloadSchema } from '@modules/appointments/presentation/http/validation/schemas/reschedule-appointment-payload.schema';
import type { IRescheduleAppointmentPayload } from '@modules/appointments/presentation/http/validation/schemas/reschedule-appointment-payload.types';
...
import { mapRescheduleAppointmentHttpResponse } from '../http-responses/map-reschedule-appointment-response';
...
import { requestBodyToRescheduleAppointmentUseCaseInput } from '../request-mappers/appointment/request-body-to-reschedule-appointment-use-case-input';
...
export class AppointmentsController {
  ...
  async getMyAppointments(
    ...
    const input = findManyParamsToGetMyAppointmentsUseCaseInput(
      params,
      user,
  ...
  async rescheduleAppointment(
    ...
    params: IIdParamPayload,
    @HttpBody(rescheduleAppointmentPayloadSchema, {
      errorMessage: 'Некорректный payload переноса записи',
    })
    payload: IRescheduleAppointmentPayload,
    @AuthenticatedUser() user: ISessionUser,
    @GetMetadata() metadata: IGetMetadata,
  ) {
    const input = requestBodyToRescheduleAppointmentUseCaseInput(
      params.id,
      payload,
      user,
      metadata.isStaffUser,
    );
    const output = await this.rescheduleAppointmentUseCase.execute(input);
    return mapRescheduleAppointmentHttpResponse(output);
  }
