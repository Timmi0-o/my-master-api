...
export class GetMasterWeeklyScheduleByIdUseCase {
  ...
  async execute(
    ...
    const entity = await this.masterWeeklyScheduleRepository.findEntityById(input.id);
    if (!entity || (!input.actor.isStaffUser && entity.deletedAt != null)) {
      throw new MasterWeeklyScheduleNotFoundError(input.id);
    }

    const item = await this.masterWeeklyScheduleRepository.findOne(input.id, input.params);
    if (!item) {
      throw new MasterWeeklyScheduleNotFoundError(input.id);
    }
    return item;
