import type { IGetMasterScheduleExceptionByIdApplicationInput } from '../../dtos/master-schedule-exception/get-master-schedule-exception-by-id.input';
import type { IGetMasterScheduleExceptionByIdApplicationOutput } from '../../dtos/master-schedule-exception/get-master-schedule-exception-by-id.output';
...
export class GetMasterScheduleExceptionByIdUseCase {
  ...
  ): Promise<IGetMasterScheduleExceptionByIdApplicationOutput> {
    const entity = await this.masterScheduleExceptionRepository.findEntityById(input.id);
    if (!entity || (!input.actor.isStaffUser && entity.deletedAt != null)) {
      throw new MasterScheduleExceptionNotFoundError(input.id);
    }

    const item = await this.masterScheduleExceptionRepository.findOne(input.id, input.params);
    if (!item) {
      throw new MasterScheduleExceptionNotFoundError(input.id);
    }
    return item;
