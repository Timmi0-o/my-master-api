...
import type { IMasterServiceFiltersPreset } from '../../validation/types/master-service-filters-preset.types';
...
export function extractMasterServiceFilter(
  ...
  const pushString = (
    ...
    value: IMasterServiceFiltersPreset['id'],
  ): void => {
    if (!value) return;
  ...
  const pushDate = (
    field: keyof IMasterServicePublicEntity & string,
    value: IMasterServiceFiltersPreset['createdAt'],
  ): void => {
    if (!value) return;
